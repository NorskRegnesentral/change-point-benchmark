# %%
"""Comparison figure for the robust change-in-mean (L1) benchmark.

Produces one subplot per data dimension, showing runtime vs. n_samples for all
algorithm variants using the same L1 cost.
...
ALGORITHM_STYLE: dict[str, tuple[str, str]] = {
    "pelt_l1": ("solid", "circle"),
    "moving_window_l1": ("dash", "square-open"),
    "binseg_l1": ("dot", "diamond"),
}

ALGORITHM_LABELS: dict[str, str] = {
    "pelt_l1": "PELT",
    "moving_window_l1": "Moving window",
    "binseg_l1": "Binary segmentation",
}

ALGORITHM_COLORS: dict[str, str] = {
    "pelt_l1": "#2ca02c",
    "moving_window_l1": "#9467bd",
    "binseg_l1": "#8c564b",
}
...
def build_figure(df: pl.DataFrame, dimensions: list[int]) -> go.Figure:
    ...
    fig.update_layout(
        title=(
            "Robust change-in-mean benchmark (L1 cost): runtime vs. sample size"
...
def build_relative_speed_figure(df: pl.DataFrame, dimensions: list[int]) -> go.Figure:
    ...
    fig.update_layout(
        title=(
            "Robust change-in-mean benchmark (L1 cost): relative runtime"
