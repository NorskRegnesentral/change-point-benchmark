#!/usr/bin/env python
"""Run the univariate continuous linear trend score null benchmark."""
...
RUNS_REGIME: list[tuple[int, int]] = [(2500, 10), (10000, 5), (50000, 3)]
RUNS_DEFAULT: int = 1
...
OUTPUT_PATH = prepare_results_path(
    f"continuous-linear-trend-benchmark_{RUN_STARTED_ON.isoformat()}_"
...
def _run_cases(cases: list[BenchmarkCase]) -> None:
    ...
    for index, case in enumerate(cases, 1):
        ...
        print(f"ski_jump={result.ski_jump_mean:.4f}s min={result.min:.4f}s")
        result_frame = pl.DataFrame([result.as_dict()])
        output_frame = (
            pl.concat([output_frame, result_frame])
...
def main() -> None:
    ...
    print("=" * 60)
    print("Continuous Linear Trend Score Null Benchmark")
    print("=" * 60)
