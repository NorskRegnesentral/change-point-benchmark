...
# ---------------------------------------------------------------------------
# Comparison pair: MovingWindow + Rank cost (multivariate only)
    ...
    *,
    ...
    for problem in problems:
        ...
        def make_sk_setup(bandwidth=bw, fit=include_fit):
            def setup(data: np.ndarray):
                det = MovingWindow(
                    change_score=CostChangeScore(RankCost()), bandwidth=bandwidth
                )
                if not fit:
                    det.fit(data)
                return (det, data), {}
...
class _CostPoisson(rpt.base.BaseCost):
...
# ---------------------------------------------------------------------------
# Comparison pair: PELT + Poisson cost
#   skchange: PELT(cost=PoissonCost())
#   ruptures: Pelt(custom_cost=_CostPoisson(), min_size=1, jump=1)
# ---------------------------------------------------------------------------


def _pair_pelt_poisson(
    problems: list[BenchmarkProblem],
    *,
    include_fit: bool = True,
    min_segment_length: int = 1,
    ...
    for problem in problems:
        cfg = problem.dataset_config
        _base_prepare = _make_prepare(problem)
        ...
        def make_rpt_setup(fit=include_fit, msl=min_segment_length):
            def setup(data: np.ndarray):
                algo = rpt.Pelt(custom_cost=_CostPoisson(), min_size=msl, jump=1)
                if not fit:
                    algo.fit(data)
                return (algo, data), {}
...
BENCHMARK_PAIRS: dict[str, Callable[..., list[BenchmarkCase]]] = {
    ...
    "pelt_1d_gaussian": _pair_pelt_gaussian,
    "pelt_poisson": _pair_pelt_poisson,
    ...
    "moving_window_rank": _pair_moving_window_rank,
    "binseg": _pair_binseg,
}
